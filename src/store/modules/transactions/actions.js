...
export default {
  ...
  getStatementByPeriod: async ({ commit }, period = { start: "", end: "" }) => {
    console.log({ commit, period });
    const authToken = localStorage.getItem('tqlWalletAppToken');
    const res = await API.Transactions.getStatementByPeriod(period, authToken);
  ...
  getUserTransactions: async ({ commit, rootState }) => {
    ...
    const res = await API.Transactions.getTransactions(authToken ? authToken : rootState.user.token);
    const { data, error, message } = await res.json();
    if (!error) {
      commit('SET_TRANSACTIONS', data);
    }
